$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        thumbnail_Min: false,//是否生成缩略图（列表小图，用于手机显示）----附加 2016年1月5日 16:41:41 （柳航）
        userphoto:false,//是否是头像 切图为150*150
        sendurl: null, //发送地址
        filetypes: "jpg,jpeg,png,gif,mp4,mp3,avi", //文件类型
        filesize: "51200", //文件大小
        btntext: "浏览...", //上传按钮的文字
    ...
    $.fn.InitUploader = function (p) {
        var fun = function (parentObj) {
            ...
            if (p.thumbnail_Min) {
                p.sendurl += "&IsThumbnail_Min=1";
            }
            if (p.userphoto) {
                p.sendurl += "&UserPhoto=1";
            }
            if (!p.multiple) {
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        parentObj.siblings(".imgpath").text(data.thumb);
                        parentObj.siblings("#idcard_img_z").val(data.thumb);
                        parentObj.siblings("#idcard_img_f").val(data.thumb);
                        parentObj.parents(".touxiang").children("img.upload-path").attr("src", data.thumb);
                        parentObj.parents(".touxiang").children("#hdimg").val(data.thumb);
