<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="cate_num_list.aspx.cs" Inherits="HT.Admin.admin.cate.cate_num_list" %>
...
<body class="mainbody">
    <form id="form1" runat="server">
        ...
        <asp:Repeater ID="rptList" runat="server">
            <HeaderTemplate>
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
                    <tr>
                        ...
                        <th align="center" width="27%">标题</th>
                    <%if (cid > 0) {%> 
                        <th align="center" width="14%">最低值</th>
                        <th align="center" width="14%">最高值</th>
                     <%}%> 
            ...
            <ItemTemplate>
                <tr>
                    ...
                    <%if (cid == 0) {%> 
                    <td align="center"><%#Eval("min")%></td>
                    <td align="center"><%#Eval("max")%></td>
                     <%}%> 
                    <td align="center"><%#Eval("sort")%></td>
                    <td align="center">
                        <%if (cid == 0)
                        { %>
                        <a href="/admin/cate/cate_num_list.aspx?cid=<%#Eval("id")%>">选项</a>
                        <%} %>
                        <a class="re_edit" data-id="<%#Eval("id")%>" data-title="<%#Eval("title")%>" data-sort="<%#Eval("sort")%>" data-min="<%#Eval("min")%>" data-max="<%#Eval("max")%>" href="javascript:;">修改</a>
                    </td>
                </tr>
            </ItemTemplate>
            <FooterTemplate>
        ...
        <div class="hide addDialog">
            ...
            <dl class="<%=(cid==0?"hide":"")%>">
                <dt>最低值：</dt>
                <dd><input type="text" class="min" name="min" placeholder="最低值" value="" /></dd>
            </dl>
            <dl class="<%=(cid==0?"hide":"")%>">
                <dt>最高值：</dt>
                <dd><input type="text" class="max" name="max" placeholder="最高值" value="" /></dd>
            </dl>
            <dl>
                <dt>排序：</dt>
...
<script type="text/javascript">
    $(function () {
        $('.add').click(function () {
            ...
            var d = dialog({
                ...
                ok: function () {
                    ...
                    var _min = $(_thisDialog.node).find('.min').val();
                    var _max = $(_thisDialog.node).find('.max').val();
                    var _sort = $(_thisDialog.node).find('.sort').val();
                    $.ajax({
                        type: 'POST',
                        url: '/admin/cate/cate_num_list.aspx',
                        dataType: 'json',
                        data: { action: 'add', cid: _cid, title: _title, min: _min, max: _max, sort: _sort },
        ...
        $('.re_edit').click(function () {
            ...
            var _omin = $(this).attr('data-min');
            var _omax = $(this).attr('data-max');
            ...
            var d = dialog({
                ...
                ok: function () {
                    ...
                    $.ajax({
                        ...
                        url: '/admin/cate/cate_num_list.aspx',
                        dataType: 'json',
                        data: { action: 'edit', id: _id, cid: _cid, title: _title, min: _min, max: _max, sort: _sort },
                        success: function (data) {
                            parent.jsprint(data.msg, '');
                            if (data.status == 1) {
                                _thisDialog.remove();
            ...
            $(d.node).find('.min').val(_omin);
            $(d.node).find('.max').val(_omax);
