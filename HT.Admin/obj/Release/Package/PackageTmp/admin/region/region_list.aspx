<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="region_list.aspx.cs" Inherits="HT.Admin.admin.region.region_list" %>
            ...
            <HeaderTemplate>
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
                    <tr>
                        ...
                        <th align="center" width="65%">标题</th>
            ...
            <ItemTemplate>
                <tr>
                    ...
                    <td align="center">
                        <%if (cid < 2)
                        { %>
                        <a href="/admin/region/region_list.aspx?pid=<%#Eval("id")%>">下级</a>
                        <%} %>
        ...
        $('.add').click(function () {
            ...
            var d = dialog({
                ...
                ok: function () {
                    ...
                    var _pid = $(_thisDialog.node).find('.pid').val();
                    var _cid = $(_thisDialog.node).find('.cid').val();
                    var _title = $(_thisDialog.node).find('.title').val();
                    var _sort = $(_thisDialog.node).find('.sort').val();
                    $.ajax({
                        type: 'POST',
                        url: '/admin/region/region_list.aspx',
                        dataType: 'json',
                        data: { action: 'add', pid: _pid, cid: _cid, title: _title, sort: _sort },
            ...
            var d = dialog({
                ...
                ok: function () {
                    ...
                    $.ajax({
                        ...
                        url: '/admin/region/region_list.aspx',
                        dataType: 'json',
                        data: { action: 'edit', id: _id, title: _title, sort: _sort },
                        success: function (data) {
                            parent.jsprint(data.msg, '');
                            if (data.status == 1) {
                                _thisDialog.remove();
